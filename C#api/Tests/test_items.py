...
class ApiItemsTests(unittest.TestCase):
    ...
    def test_7update_existing_item(self):
        ...
        self.assertTrue(
            updated_item_exists,
            "Updated item with matching Uid and Code not found in the data",
        )

    def test_7partially_update_item(self):
        partial_update = {
            "Code": "UPDATED_CODE",
            "Short_Description": "Partially Updated Test Item",
        ...
        # Fetch the item to verify the update
        response = self.client.get(f"items/{self.new_item['Uid']}")
        self.assertEqual(response.status_code, 200)

        updated_item = response.json()

        # Verify only updated fields are changed
        self.assertEqual(updated_item["Code"], partial_update["Code"])
        self.assertEqual(
            updated_item["Short_Description"], partial_update["Short_Description"]
        )
        self.assertEqual(
            updated_item["Unit_Purchase_Quantity"],
            partial_update["Unit_Purchase_Quantity"],
        )

        # Verify other fields remain unchanged
        for key, value in self.new_item.items():
            if key not in partial_update and key in updated_item:
                self.assertEqual(updated_item[key], value)
