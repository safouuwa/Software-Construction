...
class ApiItemGroupsTests(unittest.TestCase):
    ...
    def test_get_item_group_items(self):
        response = self.client.get("item_groups/1/items")
        self.assertEqual(response.status_code, 200)
    ...
    def test_7update_existing_item_group(self):
        updated_item_group = {
            ...
            "Created_At": self.new_item_group['Created_At'],
            "Updated_At": datetime.now().isoformat()  # New update time
        }

        response = self.client.put(f"item_groups/{self.new_item_group['Id']}",
                                   content=json.dumps(updated_item_group),
        ...
        updated_item_group_exists = any(
            group['Id'] == updated_item_group['Id'] and
            group['Name'] == updated_item_group['Name']
    ...
    def test_8update_non_existent_item_group(self):
        ...
        response = self.client.put("item_groups/-1",
                                   content=json.dumps(non_existent_item_group),
                                   headers={"Content-Type": "application/json"
                                            })
        self.assertEqual(response.status_code, 404)
        self.assertNotIn(non_existent_item_group,
                         self.GetJsonData("item_groups"))

    def test_9update_item_group_with_invalid_data(self):
        invalid_item_group = self.new_item_group.copy()
        invalid_item_group.pop("Id")  # Invalid because it has no Id
        response = self.client.put(f"item_groups/{self.new_item_group['Id']}",
                                   content=json.dumps(invalid_item_group),
                                   headers={"Content-Type": "application/json"
                                            })
        self.assertEqual(response.status_code, 400)
        self.assertNotIn(invalid_item_group, self.GetJsonData("item_groups"))
