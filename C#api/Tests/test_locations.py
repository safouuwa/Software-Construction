...
class ApiLocationsTests(unittest.TestCase):
    ...
    def test_7update_existing_location(self):
        updated_location = {
            ...
            "Warehouse_Id": self.new_location['Warehouse_Id'],
            "Code": "LOC002",  # Changed Code
            "Name": "Updated Location",  # Changed Name
            "Created_At": self.new_location['Created_At'],
            "Updated_At": datetime.now().isoformat()  # New update time
        }

        response = self.client.put(f"locations/{self.new_location['Id']}",
                                   content=json.dumps(updated_location),
        ...
        updated_location_exists = any(
            location['Id'] == updated_location['Id'] and
            location['Name'] == updated_location['Name']
            for location in locations_data
        )
        self.assertTrue(updated_location_exists,
                        "Updated location with matching Id and "
    ...
    def test_8update_non_existent_location(self):
        ...
        response = self.client.put("locations/-1",
                                   content=json.dumps(non_existent_location),
                                   headers={"Content-Type": "application/json"
                                            })
        self.assertEqual(response.status_code, 404)
    ...
    def test_9update_location_with_invalid_data(self):
        ...
        invalid_location.pop("Id")  # Invalid because it has no Id
        response = self.client.put(f"locations/{self.new_location['Id']}",
                                   content=json.dumps(invalid_location),
                                   headers={"Content-Type": "application/json"
                                            })
        self.assertEqual(response.status_code, 400)
        self.assertNotIn(invalid_location, self.GetJsonData("locations"))
