...
class ApiTransfersTests(unittest.TestCase):
    ...
    def test_7update_existing_transfer(self):
        ...
        response = self.client.put(f"transfers/{self.new_transfer['Id']}",
        ...
        updated_transfer_exists = any(
            transfer['Id'] == updated_transfer['Id'] and
            transfer['Reference'] == updated_transfer['Reference']
            for transfer in transfers_data
        )
        self.assertTrue(updated_transfer_exists,
                        "Updated transfer with matching Id and "
                        "Reference not found in the data")

    def test_8update_non_existent_transfer(self):
        non_existent_transfer = self.new_transfer.copy()
        non_existent_transfer["Id"] = -1
        response = self.client.put("transfers/-1",
    ...
    def test_aget_transfer_status(self):
        response = self.client.get(
            f"transfers/{self.new_transfer['Id']}/status")
        self.assertEqual(response.status_code, 200)
