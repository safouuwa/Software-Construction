...
class ApiTransfersTests(unittest.TestCase):
    ...
    def test_get_transfer_items(self):
        response = self.client.get("transfers/1/items")
        self.assertEqual(response.status_code, 200)
    ...
    def test_7update_existing_transfer(self):
        updated_transfer = {
            ...
            "Transfer_Status": "Scheduled",
            "Created_At": self.new_transfer["Created_At"],
        ...
        response = self.client.put(
            f"transfers/{self.new_transfer['Id']}",
            content=json.dumps(updated_transfer),
        ...
        updated_transfer_exists = any(
            transfer["Id"] == updated_transfer["Id"]
            and transfer["Reference"] == updated_transfer["Reference"]
            for transfer in transfers_data
        )
        self.assertTrue(
            updated_transfer_exists,
            "Updated transfer with matching Id and Reference not found in the data",
        )

    def test_10partially_update_transfer(self):
        partial_update = {
            "Status": "In Transit",
            "Notes": "Updated transfer status and notes.",
        }
        ...
        # Fetch the transfer to verify the update
        response = self.client.get(f"transfers/{self.new_transfer['Id']}")
        self.assertEqual(response.status_code, 200)

        updated_transfer = response.json()

        # Validate the updated fields
        self.assertEqual(updated_transfer["Status"], partial_update["Status"])
        self.assertEqual(updated_transfer["Notes"], partial_update["Notes"])

        # Ensure non-updated fields remain the same
        for key, value in self.new_transfer.items():
            if key not in partial_update and key in updated_transfer:
                self.assertEqual(updated_transfer[key], value)
    ...
    def test_8update_non_existent_transfer(self):
        ...
        response = self.client.put(
            "transfers/-1",
            content=json.dumps(non_existent_transfer),
            headers={"Content-Type": "application/json"},
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_9update_transfer_with_invalid_data(self):
        ...
        response = self.client.put(
            f"transfers/{self.new_transfer['Id']}",
            content=json.dumps(invalid_transfer),
            headers={"Content-Type": "application/json"},
        )
        self.assertEqual(response.status_code, 400)
        self.assertNotIn(invalid_transfer, self.GetJsonData("transfers"))
